...
(function () {
  ...
  document.addEventListener("bw:ready", () => {
    ...
    function renderRows() {
      ...
      rows.innerHTML = state.orders.map((o) => `<tr data-id="${o.id}"${o.id === state.selected ? ' class="is-selected"' : ""}><td><b>${o.number}</b><br><span class="muted small">${o.provider}</span></td><td>${when(o.createdAt)}</td><td>${BW.escapeHtml(o.email)}<br><span class="muted small">${BW.escapeHtml(o.city || "")}${o.state ? ", " + o.state : ""}</span></td><td class="small">${o.items.map(BW.escapeHtml).join("<br>")}</td><td class="num">${dollars(o.total)}</td><td>${BW.orderBadge(o.status)}</td></tr>`).join("");
    }
