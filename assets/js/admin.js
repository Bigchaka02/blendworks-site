...
(function () {
  ...
  document.addEventListener("bw:ready", () => {
    ...
    function renderRows() {
      ...
      rows.innerHTML = state.orders.map((o) => `<tr data-id="${o.id}"${o.id === state.selected ? ' class="is-selected"' : ""}><td><b>${o.number}</b><br><span class="muted small">${o.provider}${o.manual ? " · manual" : ""}</span>${o.reportedAt && o.status === "pending_payment" ? '<br><span class="badge mint">reported</span>' : ""}</td><td>${when(o.createdAt)}</td><td>${BW.escapeHtml(o.email)}<br><span class="muted small">${BW.escapeHtml(o.city || "")}${o.state ? ", " + o.state : ""}</span></td><td class="small">${o.items.map(BW.escapeHtml).join("<br>")}</td><td class="num">${dollars(o.total)}</td><td>${BW.orderBadge(o.status)}</td></tr>`).join("");
    }
    async function open(id) {
      ...
      box.innerHTML = `
        ...
        <p class="muted small">${o.provider}${o.paymentRef ? ` · payment ${BW.escapeHtml(o.paymentRef)}` : ""}${o.providerRef ? ` · ref ${BW.escapeHtml(o.providerRef)}` : ""}<br>Placed ${when(o.createdAt)}${o.paidAt ? ` · paid ${when(o.paidAt)}` : ""}</p>
        ${o.paymentInfo && o.paymentInfo.mode === "manual" ? `<div class="notice mb-1">Manual ${o.paymentInfo.provider} payment: expect <b>${dollars(o.paymentInfo.amount)}</b>${o.paymentInfo.btc ? ` = <b>${o.paymentInfo.btc} BTC</b> (quoted at $${Number(o.paymentInfo.rate).toLocaleString()})` : ""} with memo <b>${BW.escapeHtml(o.paymentInfo.memo)}</b> ${o.paymentInfo.handle ? `to ${BW.escapeHtml(o.paymentInfo.handle)}` : o.paymentInfo.address ? `to ${BW.escapeHtml(o.paymentInfo.address)}` : ""}. ${o.reportedAt ? `Customer reported sending it ${when(o.reportedAt)}.` : "Not reported by the customer yet."} Check your app/wallet, then <b>Mark paid</b>.</div>` : ""}
