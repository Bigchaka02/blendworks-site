  ...
  document.addEventListener("bw:ready", () => {
    ...
    async function open(id) {
      ...
      box.innerHTML = `
        ...
        ${o.paymentInfo && o.paymentInfo.mode === "manual" ? `<div class="notice mb-1">Manual ${BW.providerName(o.paymentInfo.provider)} payment: expect <b>${dollars(o.paymentInfo.amount)}</b>${o.paymentInfo.btc ? ` = <b>${o.paymentInfo.btc} BTC</b> (quoted at $${Number(o.paymentInfo.rate).toLocaleString()})` : ""} with memo <b>${BW.escapeHtml(o.paymentInfo.memo)}</b> ${o.paymentInfo.handle ? `to ${BW.escapeHtml(o.paymentInfo.handle)}${o.paymentInfo.name ? ` (${BW.escapeHtml(o.paymentInfo.name)})` : ""}` : o.paymentInfo.address ? `to ${BW.escapeHtml(o.paymentInfo.address)}` : ""}. ${o.reportedAt ? `Customer reported sending it ${when(o.reportedAt)}.` : "Not reported by the customer yet."} Check your app/wallet, then <b>Mark paid</b>.</div>` : ""}
